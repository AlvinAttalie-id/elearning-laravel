<x-app-layout>
    <div class="max-w-5xl px-6 py-8 mx-auto space-y-6">
        <div class="flex items-center gap-4">
            <div class="flex items-center justify-center w-12 h-12 text-green-600 bg-green-100 rounded-full">
                <i data-lucide="user-check" class="w-6 h-6"></i>
            </div>
            <div>
                <h2 class="text-2xl font-bold text-gray-800">Detail Nilai: {{ $siswa->user->name }}</h2>
                <p class="text-sm text-gray-600">Kelas: {{ $siswa->kelas->nama ?? '-' }}</p>
            </div>
        </div>

        @foreach ($jawaban as $mapel => $listJawaban)
            <div class="space-y-4">
                <h3 class="text-lg font-semibold text-indigo-700">{{ $mapel }}</h3>
        ...
        <div class="p-4 mt-6 text-sm border-l-4 border-indigo-500 rounded-md bg-indigo-50">
            Total Tugas: <strong>{{ $jumlahTugas }}</strong><br>
            Total Nilai: <strong>{{ $totalNilai }}</strong><br>
            Nilai Akhir (rata-rata x 100):
            <strong class="text-lg text-indigo-700">
